...
function renderNewRound(existingRound) {
  // existingRound: if editing pins of an existing round
  ...
  document.getElementById('createRoundBtn').textContent = isEdit ? 'Save Pins' : 'Start Round';
  // Hide date/notes when editing
  document.getElementById('roundDateInput').parentElement.querySelectorAll('p, input').forEach(el => {
    if (isEdit && (el.id === 'roundDateInput' || el.id === 'roundNotesInput' ||
                   (el.tagName === 'P' && el.textContent.match(/Date|Notes/)))) {
  ...
  pinGrid.innerHTML = '';
  // Header row: blank + P1..P5
  pinGrid.appendChild(divWith('h-label', ''));
  for (let p = 1; p <= 5; p++) {
    pinGrid.appendChild(divWith('p-header', 'P' + p));
  }
  // 18 rows
  const pins = isEdit ? [...existingRound.pins] : new Array(18).fill(null);
  for (let h = 1; h <= 18; h++) {
    pinGrid.appendChild(divWith('h-label', h.toString()));
    for (let p = 1; p <= 5; p++) {
      const cell = document.createElement('div');
      cell.className = 'pin-cell';
      cell.textContent = p;
      if (pins[h-1] === p) cell.classList.add('selected');
      cell.onclick = () => {
        pins[h-1] = p;
        pinGrid.querySelectorAll(`[data-hole="${h}"]`).forEach(c => c.classList.remove('selected'));
  ...
  document.getElementById('createRoundBtn').onclick = () => {
    const missing = pins.findIndex(p => p === null);
    if (missing !== -1) {
      toast(`Missing pin for hole ${missing + 1}`);
      return;
    }
    if (isEdit) {
